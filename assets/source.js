...
// One option is to use a form field to set an angle, and the JSON
// can reply with the setting the Servo is currently in.
// Note data.position string.
function servoStatus (arg) {
    function responseHandler(data, textStatus, jqXHR){
        if(data.status == "ok"){
            $("#response").text("Servo is in position: " + data.position);
        }
        else{
            $("#response").text("Error: " + data.path);
        }
